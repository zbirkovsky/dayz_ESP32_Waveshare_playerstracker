...
static void secondary_fetch_execute(void) {
    ...
    for (int slot = 0; slot < count && fetch_running; slot++) {
        ...
        if (err == ESP_OK) {
            ...
            if (sd_card_is_mounted() && status.players >= 0) {
                time_t now_time;
                time(&now_time);
                history_append_entry_json(server_idx, (uint32_t)now_time, (int16_t)status.players);
            }
...
static void secondary_fetch_task(void *arg) {
    ...
    secondary_fetch_execute();

    while (fetch_running) {
        // Block until notified or interval expires (replaces busy-polling)
        uint32_t notify_value = 0;
        xTaskNotifyWait(0, ULONG_MAX, &notify_value, fetch_interval);

        if (!fetch_running) break;
...
void secondary_fetch_refresh_now(void) {
    if (fetch_task_handle) {
        xTaskNotify(fetch_task_handle, 1, eSetValueWithOverwrite);
    }
}
